...
| 参数     | 默认值  | 说明                                               |
|--------|------|--------------------------------------------------|
| key    | `""` | IP配置KEY，读取配置文件 `system.allow-ip` 参数，或者使用内置预设 KEY |
| ipList | `{}` | 直接硬编码IP地址，格式为 IP 地址，或 CIDR 格式网络地址                |
...
- `INNER-IP` 内网IP地址:
  ...
  - 本机回环地址 `127.0.0.0/8` 127.0.0.0-127.255.255.255
- `INNER-IP-A` 内网A类IP地址:
  - A类地址 `10.0.0.0/8` 10.0.0.0-10.255.255.255
- `INNER-IP-B` 内网B类IP地址:
  - B类地址 `172.16.0.0/12` 172.16.0.0-172.31.255.255
- `INNER-IP-C` 内网C类IP地址:
  - C类地址 `192.168.0.0/16` 192.168.0.0-192.168.255.255
- `INNER-IP-LOCAL` 内网本机本地IP地址:
  - 本机回环地址 `127.0.0.0/8` 127.0.0.0-127.255.255.255

或者从配置文件读取，配置文件格式如下（参考：`AllowIPConfigurationProperties.allowIp`）：

```yml
system:
  common:
    allow-ip:
      office-net: # 此 `office-net` 即为 KEY
        - 127.0.0.1
        - 127.0.0.1/16
        - 192.168.0.1
        - 192.168.0.0/24
...
硬编码允许访问的IP列表，仅在 `AllowIP#key()` 为空字符串时有效，假如 `AllowIP#key()` 为空字符串且 `AllowIP#ipList()`
为空数组则直接不允许访问。

参考 `AllowIPConfigurationProperties.allowIp` 格式
